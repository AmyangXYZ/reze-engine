...
// The field layer is NOT read here. It is blitted into the HDR target inside
// the scene pass — background before geometry, foreground after — so field
// effects reach the bloom pyramid and take the same tone map as everything
// else. Sampling it here as well would draw every field effect twice, and
// compositing it here at all is what kept the mount from ever blooming.
// See passes/field-blit.ts.
...
@fragment fn fs(@builtin(position) fragCoord: vec4f) -> @location(0) vec4f {
  ...
  // The equirect, and nothing else now. Skipped behind pixels the scene fully
  // covers, where the result is multiplied by (1 - alpha) = 0 anyway.
  if (bg.w > 1.5 && sceneAlpha < 0.999) {
    {
  ...
  // Neither ribbons nor field effects are read here: all of them draw inside
  // the scene pass, so they are already in disp — tone mapped and bloomed,
  // which neither ever was while this line existed.
...
/**
 * The composite: ONE shader for every scene.
 *
 * It tone maps the HDR target and composites it over the base background — a
 * colour, a 360 equirect, or transparency. No variants and no substitutions
 * remain: effects draw inside the scene pass, so nothing here depends on which
 * of them are installed. The parameter survives because the anchor alias still
 * rides along for the scene API; it no longer changes the body.
 */
export function buildCompositeShader(effect?: CompositeEffectSource | null): string {
  const body = COMPOSITE_BODY
...
export function buildFieldShader(effect: CompositeEffectSource): string {
  ...
  return (
    ...
    /* wgsl */ `
/**
 * An effect's return, as scene light.
 *
 * NO EXPOSURE CONSTANT, and that is a measured decision rather than an
 * omission. The first attempt multiplied by 3.0, justified by the intensities
 * the particle mounts carry; decoding this engine's own AgX cube says a linear
 * 1.0 arrives at 0.77 display — light, not the mid grey that number assumed —
 * and the exposure that would preserve midtones exactly is 0.43. So 3.0 was
 * about seven times too large, and blew every falloff flat on its own.
 *
 * With no constant, an authored 1.0 means what 1.0 means EVERYWHERE else in the
 * engine: scene radiance. A mount-specific exposure would be a second meaning
 * to keep calibrated forever. Effects land slightly lifted in the midtones and
 * slightly compressed at the top, which is what a filmic transform does to
 * everything else in the frame already.
 *
 * The UPPER clamp is gone: above 1 is how a field effect reaches the bloom
 * threshold, which it could never do while its output was capped and
 * composited past the pyramid. The lower clamp stays — the layer is
 * premultiplied, so a negative channel would darken what it is drawn over.
 */
fn rzFieldOut(c: vec4f) -> vec4f {
  return vec4f(max(c.rgb, vec3f(0.0)), clamp(c.a, 0.0, 1.0));
}
