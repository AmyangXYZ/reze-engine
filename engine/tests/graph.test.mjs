...
const HAIR_BODY_INLINE = [
  ...
  "  let n_hs_001 = hue_sat_id(1.5, 1.0, 1.0, tex_color); // @node:hs_001",
  "  let n_hs_shadow = hue_sat_id(1.2, 0.5, 1.0, tex_color); // @node:hs_shadow",
...
test("cloth_smooth graph matches the hand-written shader (snapshot)", () => {
  ...
  assert.equal(
    ...
    [
      ...
      "  let n_dark_tex = hue_sat_id(1.0, 0.19999998807907104, 1.0, tex_color); // @node:dark_tex",
      "  let n_mix_004 = mix_blend(n_mix04_fac, n_dark_tex, tex_color); // @node:mix_004",
      ...
      "  let n_principled_base = hue_sat_id(1.0, 0.800000011920929, 1.0, tex_color); // @node:principled_base",
...
test("metal graph matches the hand-written shader (snapshot)", () => {
  ...
  assert.equal(
    ...
    [
      ...
      "  let n_ramp_008 = ramp_constant_edge_aa(n_str, 0.2966, vec4f(0.0, 0.0, 0.0, 1.0), vec4f(1.0, 1.0, 1.0, 1.0)); // @node:ramp_008",
      "  let n_mix04_fac = math_multiply(n_ramp_008.r, 0.5); // @node:mix04_fac",
      "  let n_tex_tint = hue_sat_id(1.0, 0.800000011920929, 1.0, tex_color); // @node:tex_tint",
...
test("stockings graph: radiance in graph, hashed alpha from alphaMode", () => {
  ...
  const expect = [
    ...
    "let n_emission_hs = hue_sat_id(1.0, 5.0, 1.0, tex_color);",
    "eval_principled(PrincipledIn(tex_color, 0.1, 1.0, 0.5, 1e+30, 0.7017999887466431, 0.5), n, l, v, sun, amb, shadow)",
