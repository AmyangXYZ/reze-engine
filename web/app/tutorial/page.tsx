...
@group(0) @binding(0) var<uniform> camera: CameraUniforms;`}
          ...
          <p className="leading-7">Now the shader can access <Inline>camera.view</Inline> and <Inline>camera.projection</Inline> directly. This uniform buffer pattern is fundamental in WebGPU—you&apos;ll use it to pass any data from CPU to GPU, including lighting parameters, material properties, and transformation matrices.</p>
        </section>
        ...
        <section className="flex flex-col items-start justify-start gap-6 w-full">
          ...
          <p className="leading-7">Now we move from a hardcoded triangle to actual model geometry. We&apos;re using a pre-parsed PMX model file—the standard format for MMD (MikuMikuDance) anime characters. The parser itself isn&apos;t covered here (any model format works; use AI to generate parsers as needed). What matters is understanding the two key data structures: vertices and indices.</p>

          <p className="leading-7">Each vertex in the model contains three types of data, stored sequentially in memory (this is called <span className="font-semibold">interleaved vertex data</span>):</p>
          <ul className="ml-6 list-disc [&>li]:mt-2">
            <li><span className="font-semibold">Position</span>: <Inline>[x, y, z]</Inline> coordinates in 3D space</li>
            <li><span className="font-semibold">Normal</span>: <Inline>[nx, ny, nz]</Inline> direction perpendicular to the surface (used for lighting)</li>
            <li><span className="font-semibold">UV coordinates</span>: <Inline>[u, v]</Inline> texture mapping coordinates (tells which part of a texture image to display)</li>
          </ul>
          <p className="leading-7">That&apos;s 8 floats per vertex (3 + 3 + 2 = 8 floats = 32 bytes). The index buffer specifies which vertices form each triangle—instead of duplicating vertex data, we reference existing vertices by their indices. This dramatically reduces memory usage.</p>

          <p className="leading-7">In <Link href={`${REPO_URL}/engines/v2.ts`} target="_blank" className="text-blue-400">engines/v2.ts</Link>, we create both vertex and index buffers from the model data:</p>
...
`}
          ...
          <p className="leading-7">The pipeline configuration changes in two ways. First, we update the vertex buffer layout to match our 32-byte stride. The shader only needs position data for now (we&apos;ll use normals and UVs in later steps), but the GPU must skip the correct number of bytes to read each position:</p>
  ...
  buffers: [{
    ...
    attributes: [{
      ...
      format: "float32x3" // read 3 floats for position
          ...
          <p className="leading-7">Second, we use indexed drawing instead of direct drawing. The render pass now calls <code className="bg-zinc-800 px-1.5 py-0.5 rounded text-sm">drawIndexed</code> and specifies the index buffer:</p>
...
pass.drawIndexed(this.model.indices.length) // draw all triangles using indices`}
          ...
          <p className="leading-7">The result is a red shape of the character. Without textures or lighting (coming next), we see only the raw geometry. But this is a major milestone—we&apos;ve gone from 3 hardcoded vertices to rendering a complex model with thousands of triangles.</p>
