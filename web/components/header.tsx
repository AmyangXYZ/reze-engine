...
export default function Header({ stats }: { stats: EngineStats | null }) {
  return (
    <header className="absolute top-0 left-0 right-0 px-4 md:px-6 py-3 md:py-4 flex items-center gap-2 z-50 w-full select-none flex flex-row justify-between">
      ...
      {stats && (
        <div className="ml-auto flex items-center gap-3 text-xs text-white/90 pointer-events-none bg-black h-7 px-3 md:py-2 md:px-4 rounded-full font-mono font-medium">
      ...
      <div className="pointer-events-auto md:hidden flex items-center gap-1 bg-black rounded-full h-7">
